...
extern std::atomic<long> v9fs_block_allocated;
extern std::atomic<long> v9fs_cache_reads;
extern std::atomic<long> v9fs_cache_misses;
#endif
...
public:
...
#if defined(V9FS_DIAGNOSTICS_ENABLED)
        v9fs_block_allocated += block_count;
#endif
    }
    ...
    int read(struct uio *uio, uint64_t offset_in_segment, uint64_t bytes_to_read) {
        print("[v9fs] [%d] -> file_cache_segment::read() i-node: %d, starting block %d, reading [%d] bytes at segment offset [%d]\n",
    ...
    int read_from_disk(struct device *device) {
        ...
        print("[v9fs] [%d] -> file_cache_segment::write() i-node: %d, starting block %d, reading [%d] blocks at disk offset [%d]\n",
              sched::thread::current()->id(), cache->inode->inode_no, starting_block, block_count_to_read, block);
        auto error = v9fs_read_blocks(device, block, block_count_to_read, data);
...
static struct file_cache *get_or_create_file_cache(struct v9fs_inode *inode, struct v9fs_super_block *sb) {
...
plan_cache_transactions(struct file_cache *cache, struct uio *uio) {
    ...
    if (cache->segments_by_index.empty() &&
        ...
        print("[v9fs] [%d] -> v9fs_cache_get_segment_operations i-node: %d, read FULL file of %d bytes\n",
    ...
    while (bytes_to_read > 0) {
        ...
        if (cache_segment != cache->segments_by_index.end()) {
            print("[v9fs] [%d] -> v9fs_cache_get_segment_operations i-node: %d, cache segment %d HIT at file offset %d\n",
            ...
            print("[v9fs] [%d] -> v9fs_cache_get_segment_operations i-node: %d, cache segment %d MISS at file offset %d\n",
                  sched::thread::current()->id(), cache->inode->inode_no, cache_segment_index, file_offset);
...
// by v9fs_read_with_cache() which in turn is called by vfs_file::read() in a critical section
...
int
cache_read(struct v9fs_inode *inode, struct device *device, struct v9fs_super_block *sb, struct uio *uio) {
    //
    ...
    print("[v9fs] [%d] v9fs_cache_read called for i-node [%d] at %d with %d ops\n",
    ...
    print("[v9fs] [%d] v9fs_cache_read completed for i-node [%d]\n", sched::thread::current()->id(),
          inode->inode_no);
    return error;
}
