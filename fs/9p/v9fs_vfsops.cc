...
/*
 * Mount a file system
 * 创建9p连接,设置mp的m_data, m_root->d_vnode
 * [OK]
 */
static int v9fs_mount(struct mount *mp, const char *dev, int flags, const void *data)
{
    ...
    /* 当前fid即为访问的根节点,mp->m_root现在是一个空白的dentry
     * 读取root的vnode属性进行填充
     */
...
static int v9fs_sync(struct mount *mp)
{
    struct p9_fid *fid = ((struct v9fs_inode *) (mp->m_root->d_vnode->v_data))->fid;
    ...
    else
    {
        ...
        */
        debugf("V9FS: Unsupported file system operation: sync\n");
    }
